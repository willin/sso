...
import { type AppLoadContext, type SessionStorage } from '@remix-run/cloudflare-pages';
...
> extends Strategy<User, OAuth2StrategyVerifyParams<Profile, ExtraParams>> {
  ...
  async authenticate(
    ...
    options: AuthenticateOptions & { state?: string }
  ): Promise<User> {
    let url = new URL(request.url);
    let state = options?.state ?? nanoid(32);
